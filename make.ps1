...
function Dev-Setup {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "エラー: Dockerが起動していません。Docker DesktopまたはRancher Desktopを起動してから再度実行してください。" -ForegroundColor Red
        Write-Host $dockerInfo -ForegroundColor Red
        exit 1
    }

    # 3.5. 既存のコンテナをクリーンアップ
    Write-Host "3.5. 既存のコンテナを確認しています..." -ForegroundColor Yellow
    $existingContainers = docker ps -a --filter "name=app-base-db" --format "{{.Names}}" 2>&1
    if ($existingContainers -and $LASTEXITCODE -eq 0) {
        foreach ($container in $existingContainers) {
            if ($container) {
                Write-Host "既存のコンテナを削除しています: $container" -ForegroundColor Yellow
    ...
    if ($LASTEXITCODE -ne 0) {
        # ポート競合エラーの場合、より詳細な情報を表示
        if ($dockerUp -match "port is already allocated") {
            Write-Host "エラー: ポート5433が既に使用されています。" -ForegroundColor Red
            Write-Host "以下のいずれかの方法で解決してください：" -ForegroundColor Yellow
            Write-Host "  1. ポート5433を使用しているコンテナを停止: docker ps | findstr 5433" -ForegroundColor Yellow
            Write-Host "  2. ポート5433を使用しているプロセスを確認: netstat -ano | findstr :5433" -ForegroundColor Yellow
        }
